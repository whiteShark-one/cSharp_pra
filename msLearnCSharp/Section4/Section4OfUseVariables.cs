            ...
            do
            {
                ...
                switch (menuSelection)
                {
                    ...
                    case "2":
                        ...
                        // #3 update to "rotating" animation with countdown
                        string[] searchingIcons = {" |", " /", "--", " \\", " *"};

                        // #4 loop through the ourAnimals array to search for matching animals
                        for (int i = 0; i < maxPets; i++)
                        {
                            if (ourAnimals[i, 1].Contains("dog"))
                            {
                                ...
                                foreach(string term in dogSearches)
                                {
                                    if (term != null && term.Trim() != "")
                                    {
                                        ...
                                        for (int j = 2; j > -1; j--)
                                        {
                                            ...
                                            {
                                                // \r 的主要作用是将光标移回当前行的开头，而不会移动到下一行
                                                Console.Write($"\rsearching our dog {ourAnimals[i,3]} for {term.Trim()} {icon} {j.ToString()}");
                                                Thread.Sleep(100);
                                            }
                                            // 清空当前一行文字，即生成一整行、填满控制台宽度的空白字符串（一整行空格）
                                            Console.Write($"\r{new String(' ', Console.BufferWidth)}");
                                        }
                                    }
                                    if (dogDescription.Contains(" " + term.Trim() + " "))
                                    {
                                        Console.WriteLine($"Our dog {ourAnimals[i, 3]} matches your search for {term.Trim()}");
